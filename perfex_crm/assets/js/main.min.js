...
function kanban_load_more(t, a, e, i, n) {
	...
	if (d <= r) {
		...
		0 != c.length && "" !== c.val() && (o.sort_by = c.val(), o.sort = p), void 0 !== l && "" !== l && (o.search = l), $.each($("#kanban-params input"), function() {
			...
			d++, $('[data-load-status="' + t + '"]').before(e), $('[data-col-status-id="' + t + '"]').attr("data-total", parseInt($('[data-col-status-id="' + t + '"]').attr("data-total")) + $("<ul></ul>").append(e).find("li").length), $(a).attr("data-page", d), fix_kanban_height(i, n)
		}).fail(function(e) {
...
function leads_kanban_update(t, e) {
	if (e === t.item.parent()[0]) {
		var i = {
			status: $(t.item.parent()[0]).attr("data-lead-status-id"),
			leadid: $(t.item).attr("data-lead-id"),
			order: []
		};
		$.each($(t.item).parents(".leads-status").find("li"), function(e, t) {
			var a = $(t).attr("data-lead-id");
			a && i.order.push([a, e + 1])
		}), setTimeout(function() {
			$.post(admin_url + "leads/update_lead_status", i).done(function(e) {
				update_kan_ban_total_when_moving(t, i.status), leads_kanban()
			})
		}, 200)
	}
}
...
function remove_proposal_comment(t) {
	confirm_delete() && requestGetJSON("proposals/remove_comment/" + t).done(function(e) {
		1 == e.success && ($('[data-commentid="' + t + '"]').remove(), update_comments_count("proposal"))
	})
}
...
function tasks_kanban_update(t, e) {
	if (e === t.item.parent()[0]) {
		var a = $(t.item.parent()[0]).attr("data-task-status-id"),
			i = {
				order: [],
				status: a
			};
		$.each($(t.item.parent()[0]).find("[data-task-id]"), function(e, t) {
			var a = $(t).attr("data-task-id");
			a && i.order.push([a, e + 1])
		}), task_mark_as(a, $(t.item).attr("data-task-id")), check_kanban_empty_col("[data-task-id]"), setTimeout(function() {
			$.post(admin_url + "tasks/update_order", i).done(function(e) {
				update_kan_ban_total_when_moving(t, i.status), tasks_kanban()
			})
		}, 200)
	}
}
...
function add_item_to_table(n, s, e, o) {
	if ("" !== (n = void 0 === n || "undefined" == n ? get_item_preview_values() : n).description || "" !== n.long_description || "" !== n.rate) {
		...
		var d = /<br[^>]*>/gi;
		return get_taxes_dropdown_template(t, n.taxname).done(function(e) {
			p += '<input type="hidden" class="order" name="newitems[' + _ + '][order]">', p += "</td>", p += '<td class="bold description"><textarea name="newitems[' + _ + '][description]" class="form-control" rows="5">' + n.description + "</textarea></td>", p += '<td><textarea name="newitems[' + _ + '][long_description]" class="form-control item_long_description" rows="5">' + n.long_description.replace(d, "\n") + "</textarea></td>";
...
function estimates_pipeline_update(t, e) {
	if (e === t.item.parent()[0]) {
		var i = {
			estimateid: $(t.item).attr("data-estimate-id"),
			status: $(t.item.parent()[0]).attr("data-status-id"),
			order: []
		};
		$.each($(t.item).parents(".pipeline-status").find("li"), function(e, t) {
			var a = $(t).attr("data-estimate-id");
			a && i.order.push([a, e + 1])
		}), check_kanban_empty_col("[data-estimate-id]"), setTimeout(function() {
			$.post(admin_url + "estimates/update_pipeline", i).done(function(e) {
				update_kan_ban_total_when_moving(t, i.status), estimate_pipeline()
			})
		}, 200)
	}
}

function proposals_pipeline_update(t, e) {
	if (e === t.item.parent()[0]) {
		var i = {
			order: [],
			status: $(t.item.parent()[0]).attr("data-status-id"),
			proposalid: $(t.item).attr("data-proposal-id")
		};
		$.each($(t.item).parents(".pipeline-status").find("li"), function(e, t) {
			var a = $(t).attr("data-proposal-id");
			a && i.order.push([a, e + 1])
		}), check_kanban_empty_col("[data-proposal-id]"), setTimeout(function() {
			$.post(admin_url + "proposals/update_pipeline", i).done(function(e) {
				update_kan_ban_total_when_moving(t, i.status), proposals_pipeline()
			})
		}, 200)
	}
}
		...
		e.on("changed.bs.select", function() {
			...
			$(this).find('[name^="invoice"]').filter('select[name$="[paymentmode]"],input[name$="[amount]"]').each(function() {
				...
				t.rules("add", {
					required: function() {
						var e = !1;
						return t.closest(".batch_payment_item").find("input, select").filter('select[name$="[paymentmode]"],input[name$="[transactionid]"],input[name$="[amount]"]').each(function() {
							"" != $(this).val() && (e = !0), $(this).hasClass("selectpicker") && e && (t.prop("required", !0), $(this).selectpicker("refresh"))
						}), e
					}
				})
			})
